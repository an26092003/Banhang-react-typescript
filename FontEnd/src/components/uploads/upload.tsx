...
const UploadFile: FC<Props> = ({ setImages, images }) => {
    ...
    const handleChange = (e: SyntheticEvent) => {
      ...
      if (files.length > 0) {
          const file = files[0]; // Chỉ lấy ảnh đầu tiên từ files
  
          const preview = Object.assign(file, {
              preview: URL.createObjectURL(file),
          });
  
          setFileList([preview]); // Chỉ đặt ảnh đầu tiên vào fileList
          setImages([]); // Đặt lại images về rỗng khi chọn ảnh mới
      }
  };
